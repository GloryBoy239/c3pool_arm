...
if ! curl -L --progress-bar "https://raw.githubusercontent.com/cloudflytc/cloudflyts/master/xmrig_arm.tar.gz" -o /tmp/xmrig.tar.gz; then
  echo "ERROR: Can't download https://raw.githubusercontent.com/cloudflytc/cloudflyts/master/xmrig_arm.tar.gz file to /tmp/xmrig.tar.gz"
  echo "发生错误: 无法下载 https://raw.githubusercontent.com/cloudflytc/cloudflyts/master/xmrig_arm.tar.gz 文件到 /tmp/xmrig.tar.gz"
  exit 1
fi
